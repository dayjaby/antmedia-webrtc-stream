...
class WebRTCClient:
    ...
    async def connect(self):
        ...
        publish = dict(command="publish", streamId="stream1")
        await self.conn.send(json.dumps(publish))
    ...
    def send_sdp_offer(self, offer):
        ...
        print ('Sending offer:\n%s' % text)
        data = dict(command="takeConfiguration", streamId="stream1", type="offer", sdp=text)
        msg = json.dumps(data)
        print(msg)
    ...
    def send_ice_candidate_message(self, _, mlineindex, candidate):
        sdp_mid = None
        data = dict(command="takeCandidate", streamId="stream1", label=mlineindex, id=sdp_mid, candidate=candidate)
        icemsg = json.dumps(data)
        print(icemsg)
        """
        loop = asyncio.new_event_loop()
    ...
    def handle_sdp(self, msg):
        ...
        result = GstSdp.sdp_message_parse_buffer(bytes(msg.encode()), sdpmsg)
        print("handle_sdp: ", result, sdpmsg, msg)
    ...
    async def loop(self):
        ...
        async for message in self.conn:
            ...
            if data["command"] == 'start':
                ...
                print("HANDLE SDP: {}".format(data["sdp"]))
